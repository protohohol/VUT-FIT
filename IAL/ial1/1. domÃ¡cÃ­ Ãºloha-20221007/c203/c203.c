...
void Queue_Init( Queue *queue ) {
	if (queue == NULL) { //checking if queue is not NULL
		Queue_Error(QERR_INIT);
	} else {
		for (int i = 0; i < QUEUE_SIZE; i++) { //every element of array is '*' now
			queue->array[i] = '*';
		}
		queue->firstIndex = 0; //indexes are 0 now
		queue->freeIndex = 0;
	}
}
...
int nextIndex( int index ) {
	return (index + 1) % QUEUE_SIZE; //goes one by one to the penultimate index and starts over
}
...
int Queue_IsEmpty( const Queue *queue ) {
	//checks if free index is first index (it means, that queue is empty)
...
int Queue_IsFull( const Queue *queue ) {
	return nextIndex(queue->freeIndex) == 0 ? 1 : 0; //checks if next index is 0 (it means, that queue is full)
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	if (Queue_IsEmpty(queue)) {
		...
		*dataPtr = queue->array[queue->firstIndex]; //returns char from the beggining of queue
	}
}
...
void Queue_Remove( Queue *queue ) {
	if (Queue_IsEmpty(queue)) {
		...
		queue->firstIndex = nextIndex(queue->firstIndex); //replaces first index with next index
	}
}
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	if (Queue_IsEmpty(queue)) {
		...
		Queue_Front(queue, dataPtr); //takes first char from queue
		Queue_Remove(queue); //pushes firstIndex by one
	}
}
...
void Queue_Enqueue( Queue *queue, char data ) {
	if (Queue_IsFull(queue)) {
		...
		queue->array[queue->freeIndex] = data; //inserts data to free index
		queue->freeIndex = nextIndex(queue->freeIndex); //pushes freeIndex by one
	}
}
